...
(def HISTORY-COUNT 25)
(def STRINGS-COUNT 25)
...
(defn add-to-history [value]
  (let [entry {:value value :timestamp (.now js/Date)}]
    (swap! history-state
      (fn [old-history]
        (vec (take HISTORY-COUNT (cons entry old-history)))))))
...
(defn Strings [chance]
  (let
    [length 25
     state (r/atom
             {:chance chance
              :values (generate-strings chance length STRINGS-COUNT)})
     copy (fn [index value]
            (let [replacement (first (generate-strings (:chance @state) length 1))]
       ...
       (fn [this old-argv]
         (let
           ...
           (if (and new-chance (not= old-chance new-chance))
             (let
               [new-values (generate-strings new-chance length STRINGS-COUNT)]
               (swap! state assoc :chance new-chance :values new-values)))))
